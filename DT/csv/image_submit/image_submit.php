<?php	
/*	
* ファイルパス(Win)：C:\xampp\htdocs\DT\csv\csv.php	
...
* CSVファイルの読み込みは、PHP5.1以上だと、SplFileObject(クラス)を利用すると良い。	
...
* if ( is_null( $line[0] ) ) continue; とかでもOK。あと、fgetcsvのサンプル内のpreg_matchの正規表現で、	
* ^[0-9]{1}$となっていますが、[0-9]が一文字を表しているので、{1}はいらないです。説明として残しています。	
*/	
// クラスについては、4日目の講座でご説明します	
// オブジェクトからインスタンス生成	
// SplFileObject:ファイルを読み込む定義済みクラス	
// https://www.php.net/manual/ja/splfileobject.construct.php	
// SplFileObject::READ_CSV : 定義済みの定数。CSV 列として行を読み込む	
...
// exit;	
if ($line[0] === null) continue; // line[0](先頭行)がnullの場合、スキップして処理続行。continue省くと97行目(nullの行)でエラーが出る	
$divDate = explode('-', $line[1]);	
// $divDate = ['2011','12','08'];	
// preg_match：正規表現によるマッチングを行う	
// trim：文字列の先頭および末尾にあるスペースを取り除く	
// checkdate：「月」「日」「年」の順番	
if (preg_match('/^[0-9]{4}$/', trim($line[0])) === 0 || // preg_matchはマッチした回数を返す(0回)	
checkdate($divDate[1], $divDate[2], $divDate[0]) === false ||	
preg_match('/^[0-9]$/', $line[2]) === 0) { // 一桁の場合、上記の通り{1}不要	
